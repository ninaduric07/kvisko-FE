...
function parseValue(value, field) {
    if (field && "number" === field.dataType && type_1.isString(value)) {
        return Number(value)
    }
...
function getFiltersForExpandedDimension(options) {
    return module_widget_utils_1.getFiltersByPath(options[options.headerName], options.path).concat(module_widget_utils_1.getFiltersByPath(options["rows" === options.headerName ? "columns" : "rows"], options.oppositePath || []))
}
...
var RemoteStore = class_1.default.inherit({
    ...
    createDrillDownDataSource: function(loadOptions, params) {
        ...
        var filters = module_widget_utils_1.getFiltersByPath(loadOptions.rows, params.rowPath).concat(module_widget_utils_1.getFiltersByPath(loadOptions.columns, params.columnPath)).concat(getFiltersForDimension(loadOptions.rows)).concat(loadOptions.filters || []).concat(getFiltersForDimension(loadOptions.columns));
