/**
 * DevExtreme (bundles/__internal/grids/pivot_grid/local_store/module.js)
 * Version: 22.2.6
...
var custom_store_1 = __importDefault(require("../../../../data/custom_store"));
...
var PATH_DELIMETER = "/./";
var LocalStore = class_1.default.inherit(function() {
    ...
    function getDataSelector(dataField) {
        return -1 !== dataField.indexOf(".") ? data_1.compileGetter(dataField) : function(data) {
    ...
    function getDateValue(dataSelector) {
        return function(data) {
            ...
            if (value && !(value instanceof Date)) {
                value = date_serialization_1.default.deserializeDate(value)
            }
            return value
        }
    }
        ...
        iterator_1.each(fields || [], (function(_, field) {
            ...
            if (!field.selector) {
                ...
                if ("date" === field.dataType) {
                    ...
                    var valueSelector_1 = getDateValue(dataSelector);
                    fieldSelector = function(data) {
                        var value = valueSelector_1(data);
                        return intervalSelector ? intervalSelector(value) : value
                    }
                } else if ("number" === field.dataType) {
                    groupInterval = type_1.isNumeric(field.groupInterval) && field.groupInterval > 0 && field.groupInterval;
                    fieldSelector = function(data) {
                        var value = dataSelector(data);
                ...
                module_widget_utils_1.setFieldProperty(field, "selector", fieldSelector)
            }
        }))
    }
    ...
    function getAggregator(field) {
        if ("custom" === field.summaryType) {
            field.calculateCustomSummary = field.calculateCustomSummary || common_1.noop;
            return {
                seed: function() {
                    var options = {
        ...
        return utils_1.aggregators[field.summaryType] || utils_1.aggregators.count
    }
    ...
    function aggregationStep(measures, aggregationCells, data) {
        for (var aggregatorIndex = 0; aggregatorIndex < measures.length; aggregatorIndex += 1) {
            ...
            for (var cellSetIndex = 0; cellSetIndex < aggregationCells.length; cellSetIndex += 1) {
                ...
                } else if (type_1.isDefined(cellValue)) {
    ...
    function createDimensionFilters(dimension) {
        var filters = [];
        iterator_1.each(dimension, (function(_, field) {
            ...
            filterValues.length && filters.push((function(dataItem) {
                var value = field.levels ? function(levels, data) {
                    var value = [];
                    iterator_1.each(levels, (function(_, field) {
        ...
        if (expandedDimensions) {
            filters.push((function(dataItem) {
                ...
                for (var i = 0; i < path.length; i += 1) {
                    ...
                    if (data_1.toComparable(expandValue, true) !== data_1.toComparable(path[i], true)) {
                        return false
                    }
                }
                return true
    ...
    function loadCore(items, options, notifyProgress) {
        ...
        ! function processData() {
            ...
            measures = options.values, cells = values, void iterator_1.each(measures, (function(aggregatorIndex, cellField) {
                var aggregator = getAggregator(cellField);
                if (aggregator.finalize) {
                    iterator_1.each(cells, (function(_, row) {
                        iterator_1.each(row, (function(_, cell) {
    ...
    function filterDataSource(dataSource, fieldSelectors) {
        ...
        if (dataSource.store() instanceof custom_store_1.default && filter) {
            filter = processFilter(filter, fieldSelectors);
            return query_1.default(dataSource.items()).filter(filter).toArray()
        }
    ...
    function loadDataSource(dataSource, fieldSelectors, reload) {
        ...
        var customizeStoreLoadOptionsHandler = function(options) {
            if (dataSource.store() instanceof array_store_1.default) {
        ...
        if (!dataSource.isLoaded() || reload) {
            ...
            deferred_1.when(loadDeferred).done((function() {
    ...
    function fillSelectorsByFields(selectors, fields) {
        fields.forEach((function(field) {
            if (field.dataField && "date" === field.dataType) {
                var valueSelector_2 = getDateValue(getDataSelector(field.dataField));
                selectors[field.dataField] = function(data) {
                    return valueSelector_2(data)
                }
            }
        }))
    }
    ...
    function processFilter(filter, fieldSelectors) {
        ...
        if (type_1.isString(filter[0]) && (filter[1] instanceof Date || filter[2] instanceof Date)) {
    ...
    return {
        ctor: function(options) {
            this._progressChanged = options.onProgressChanged || common_1.noop;
        ...
        getFields: function(fields) {
            ...
            loadDataSource(dataSource, getFieldSelectors(fields)).done((function(data) {
                d.resolve(module_widget_utils_1.discoverObjectFields(data, fields))
            })).fail(d.reject);
            return d
        },
        key: function() {
        ...
        load: function(options) {
            ...
            loadDataSource(dataSource, getFieldSelectors(options), options.reload).done((function(data) {
                deferred_1.when(loadCore(data, options, that._progressChanged)).done(d.resolve)
            })).fail(d.reject);
            return d
        },
        filter: function() {
        ...
        getDrillDownItems: function(loadOptions, params) {
            ...
            var pathFilter = createFilter({
                rows: module_widget_utils_1.getFiltersByPath(loadOptions.rows, params.rowPath),
                columns: module_widget_utils_1.getFiltersByPath(loadOptions.columns, params.columnPath),
                filters: []
            });
