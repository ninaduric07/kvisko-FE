/**
 * DevExtreme (bundles/__internal/grids/pivot_grid/xmla_store/module.js)
 * Version: 22.2.6
...
var XmlaStore = class_1.default.inherit(function() {
    ...
    function crossJoinElements(elements) {
        ...
        return elements.length > 1 ? string_1.format("CrossJoin({0})", elementsString) : elementsString
    }
                ...
                if (0 === i && 0 === expandAllCount) {
                    ...
                    if (isLastDimensionInGroup || i === expandAllIndex) {
                        if (prevHierarchyName === hierarchyName) {
                            ...
                            arg = (level = prevDimension.dataField, levelMember = member, nextLevel = dataField, memberExpression = void 0, memberExpression = levelMember || level, "Descendants({" + memberExpression + "}, " + nextLevel + ", SELF_AND_BEFORE)")
                        } else {
    ...
    function fillCrossJoins(crossJoins, path, expandLevel, expandIndex, slicePath, options, axisName, cellsString, take, totalsOnly) {
        ...
        do {
            ...
            if (!take && !totalsOnly) {
                crossJoin = string_1.format("NonEmpty({0}, {1})", crossJoin, cellsString)
            }
    ...
    function declare(expression, withArray, name, type) {
        ...
        type = type || "set";
        withArray.push(string_1.format("{0} {1} as {2}", type, name, expression));
        return name
    }
    ...
    function generateAxisMdx(options, axisName, cells, withArray, parseOptions) {
        ...
        var expandLevel = 0;
        var result = [];
        var cellsString = string_1.format("{{0}}", cells.join(","));
        ...
        if (crossJoins.length) {
            ...
            if ("rows" === axisName && options.rowTake) {
                expression = string_1.format("Subset({0}, {1}, {2})", expression, options.rowSkip > 0 ? options.rowSkip + 1 : 0, options.rowSkip > 0 ? options.rowTake : options.rowTake + 1)
            }
            if ("columns" === axisName && options.columnTake) {
                expression = string_1.format("Subset({0}, {1}, {2})", expression, options.columnSkip > 0 ? options.columnSkip + 1 : 0, options.columnSkip > 0 ? options.columnTake : options.columnTake + 1)
            }
        ...
        return string_1.format("{0} DIMENSION PROPERTIES PARENT_UNIQUE_NAME,HIERARCHY_UNIQUE_NAME, MEMBER_VALUE ON {1}", crossJoinElements(result), axisName)
    }
    ...
    function generateAxisFieldsFilter(fields) {
        ...
        iterator_1.each(fields, (function(_, field) {
            ...
            if (filterValues.length) {
                filterStringExpression = string_1.format("{{0}}", filterExpression.join(","));
                if ("exclude" === field.filterType) {
    ...
    function generateMdxCore(axisStrings, withArray, columns, rows, filters, slice, cubeName, options) {
        ...
        if (axisStrings.length) {
            ...
            mdxString = withString + string_1.format("SELECT {2} FROM {0} {1} CELL PROPERTIES VALUE, FORMAT_STRING, LANGUAGE, BACK_COLOR, FORE_COLOR, FONT_FLAGS", function(columnsFilter, rowsFilter, filter, cubeName) {
                var from = "[" + cubeName + "]";
                iterator_1.each([columnsFilter, rowsFilter, filter], (function(_, filter) {
                    if (filter) {
                        from = string_1.format("(SELECT {0} FROM {1})", filter + "on 0", from)
                    }
                }));
                return from
            }(generateAxisFieldsFilter(columns), generateAxisFieldsFilter(rows), generateAxisFieldsFilter(filters || []), cubeName), slice.length ? string_1.format("WHERE ({0})", slice.join(",")) : "", select)
        }
        return mdxString
    }
    ...
    function fillDataSourceAxes(dataSourceAxis, axisTuples, measureCount, visibleLevels) {
        ...
        iterator_1.each(axisTuples, (function(tupleIndex, members) {
            ...
            var dataIndex = type_1.isDefined(measureCount) ? Math.floor(tupleIndex / measureCount) : tupleIndex;
    ...
    function parseResult(xml, parseOptions) {
        ...
        var axes = function(xml, skipValues) {
            ...
            iterator_1.each(xml.getElementsByTagName("Axis"), (function(_, axisElement) {
                ...
                var axis = [];
                var index = 0;
                if (0 === name.indexOf("Axis") && type_1.isNumeric(getNumber(name.substr(4)))) {
                    axes.push(axis);
                    iterator_1.each(axisElement.getElementsByTagName("Tuple"), (function(_, tupleElement) {
                        ...
                        for (var i = membersCount; i >= 0; i -= 1) {
                            ...
                            members[i] = {
                                ...
                                value: (valueText = getFirstChildText(tuple, "MEMBER_VALUE"), type_1.isNumeric(valueText) ? parseFloat(valueText) : valueText),
                                level: level,
                                index: index++,
    ...
    return {
        ...
        getFields: function() {
            ...
            deferred_1.when(dimensionsRequest, measuresRequest, hierarchiesRequest, levelsRequest).then((function(dimensionsResponse, measuresResponse, hierarchiesResponse, levelsResponse) {
                execXMLA(options, string_1.format(discover, catalog, cube, "MDSCHEMA_MEASUREGROUPS", localeIdProperty)).done((function(measureGroupsResponse) {
        ...
        load: function(options) {
            ...
            var load = function() {
                if (mdxString) {
                    deferred_1.when(sendQuery(storeOptions, mdxString), rowCountMdx && sendQuery(storeOptions, rowCountMdx)).done((function(executeXml, rowCountXml) {
