/**
 * DevExtreme (bundles/__internal/grids/tree_list/data_source_adapter/module.js)
 * Version: 22.2.6
...
var queryByOptions = store_helper_1.default.queryByOptions;
var DEFAULT_KEY_EXPRESSION = "id";
var isFullBranchFilterMode = function(that) {
    return "fullBranch" === that.option("filterMode")
};
var DataSourceAdapterTreeList = ui_grid_core_data_source_adapter_1.default.inherit({
    _createKeyGetter: function() {
        var keyExpr = this.getKeyExpr();
        return data_1.compileGetter(keyExpr)
    },
    _createKeySetter: function() {
        var keyExpr = this.getKeyExpr();
        if (type_1.isFunction(keyExpr)) {
            return keyExpr
        }
        return data_1.compileSetter(keyExpr)
    },
    createParentIdGetter: function() {
        return data_1.compileGetter(this.option("parentIdExpr"))
    },
    createParentIdSetter: function() {
        var parentIdExpr = this.option("parentIdExpr");
        if (type_1.isFunction(parentIdExpr)) {
            return parentIdExpr
        }
        return data_1.compileSetter(parentIdExpr)
    },
    _createItemsGetter: function() {
        return data_1.compileGetter(this.option("itemsExpr"))
    },
    _createHasItemsGetter: function() {
        var hasItemsExpr = this.option("hasItemsExpr");
        return hasItemsExpr && data_1.compileGetter(hasItemsExpr)
    },
    _createHasItemsSetter: function() {
        var hasItemsExpr = this.option("hasItemsExpr");
        if (type_1.isFunction(hasItemsExpr)) {
            return hasItemsExpr
        }
        return hasItemsExpr && data_1.compileSetter(hasItemsExpr)
    },
    ...
    _convertItemToNode: function(item, rootValue, nodeByKey) {
        ...
        parentId = type_1.isDefined(parentId) ? parentId : rootValue;
    ...
    _convertDataToPlainStructure: function(data, parentId, result) {
        ...
        if (this._itemsGetter && !data.isConverted) {
            ...
            for (var i = 0; i < data.length; i++) {
                var item = array_utils_1.createObjectWithChanges(data[i]);
                ...
                if (childItems && childItems.length) {
                    ...
                    if (!type_1.isFunction(itemsExpr)) {
    ...
    _createIdFilter: function(field, keys) {
        ...
        return ui_grid_core_utils_1.default.combineFilters(parentIdFilters, "or")
    },
    ...
    _loadParentsOrChildren: function(data, options, needChildren) {
        ...
        var _a = that._generateInfoToLoad(data, needChildren),
            keys = _a.keys,
            keyMap = _a.keyMap;
        ...
        loadOptions = extend_1.extend({}, loadOptions, {
            filter: !needLocalFiltering ? filter : null
        });
        var store = options.fullData ? new array_store_1.default(options.fullData) : that._dataSource.store();
        that.loadFromStore(loadOptions, store).done((function(loadedData) {
            if (loadedData.length) {
                if (needLocalFiltering) {
                    loadedData = query_1.default(loadedData).filter(filter).toArray()
                }
    ...
    _processChanges: function(changes) {
        ...
        changes.forEach((function(change) {
            if ("insert" === change.type) {
                processedChanges = processedChanges.concat(_this._applyInsert(change))
            } else if ("remove" === change.type) {
                processedChanges = processedChanges.concat(_this._applyRemove(change))
    ...
    _handleChanging: function(e) {
        ...
        var processChanges = function(changes) {
            ...
            return _this._processChanges(changesToProcess)
        };
    ...
    _handleDataLoaded: function(options) {
        ...
        if (!options.remoteOperations.filtering && options.loadOptions.filter) {
            options.fullData = queryByOptions(query_1.default(options.data), {
            ...
            if (node.visible && node.hasChildren) {
                if (isFullBranch) {
                    if (node.children.filter((function(node) {
                        ...
                        module_core_1.default.foreachNodes(node.children, (function(node) {
                            node.visible = true
                        }))
                    }
                } else {
    ...
    _processTreeStructure: function(options, visibleItems) {
        ...
        if (parentIds && parentIds.length || this._isReload) {
            ...
            if (!this._rootNode) {
                options.data = (new deferred_1.Deferred).reject(ui_errors_1.default.Error("E1046", this.getKeyExpr()));
                return
            }
    ...
    _handleDataLoadedCore: function(options) {
        ...
        if (!options.isCustomLoading) {
            if (needLoadParents) {
                var d_1 = options.data = new deferred_1.Deferred;
    ...
    getKeyExpr: function() {
        ...
        if (type_1.isDefined(key) && type_1.isDefined(keyExpr)) {
            if (!common_1.equalByValue(key, keyExpr)) {
                throw ui_errors_1.default.Error("E1044")
            }
        }
    ...
    isRowExpanded: function(key, cache) {
        if (cache) {
            var isExpandedByKey_1 = cache.isExpandedByKey;
            if (!isExpandedByKey_1) {
                isExpandedByKey_1 = cache.isExpandedByKey = {};
                this.option("expandedRowKeys").forEach((function(key) {
                    isExpandedByKey_1[key] = true
                }))
            }
            return !!isExpandedByKey_1[key]
        }
        var indexExpandedNodeKey = ui_grid_core_utils_1.default.getIndexByKey(key, this.option("expandedRowKeys"), null);
        return indexExpandedNodeKey >= 0
    },
    _changeRowExpandCore: function(key) {
        var expandedRowKeys = this.option("expandedRowKeys").slice();
        var indexExpandedNodeKey = ui_grid_core_utils_1.default.getIndexByKey(key, expandedRowKeys, null);
    ...
    changeRowExpand: function(key) {
        ...
        return this._isNodesInitializing ? (new deferred_1.Deferred).resolve() : this.load()
    },
    ...
    getNodeLeafKeys: function() {
        ...
        keys.forEach((function(key) {
            ...
            node && module_core_1.default.foreachNodes([node], (function(childNode) {
    ...
    getChildNodeKeys: function(parentKey) {
        ...
        var childrenKeys = [];
        node && module_core_1.default.foreachNodes(node.children, (function(childNode) {
    ...
    forEachNode: function() {
        ...
        module_core_1.default.foreachNodes(nodes, callback)
    }
});
exports.default = {
