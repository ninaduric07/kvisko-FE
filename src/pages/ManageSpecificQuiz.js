...
const ManageSpecificQuiz = () => {
    ...
    React.useEffect(() => {
        ...
        const getQuiz = async () => {
            let data2 = (await (axios.get(process.env.REACT_APP_BASE_URL +'/admin/getQuiz/' + sifKviz))).data;
            setKviz(data2);
            setImages([data2.image]);
            setName(data2.name);
            setDescription(data2.description);
            setCategory(data2.sifKategorije);
            setDateAndTimeOpen(data2.availableFrom);
            setDateAndTimeClose(data2.availableTo);
            setDuration(data2.durationSec);
            setReviewDuration(data2.reviewDurationSec);
        };
        getData().catch((err) => console.log(err));
        getQuiz().catch((err) => console.log(err));
    ...
    const successMessage = () => {
        return (
            <div
                ...
                <p style={{ color: "green", fontSize: 20 }}>Uspješno ste uredili kviz!</p>
            </div>
        );
    };
    ...
    const errorMessage = () => {
        return (
            <div
                ...
                <p style={{ color: "red", fontSize: 20 }}>Niste uspjeli urediti kviz</p>
            </div>
        );
    };
    ...
    const onSubmitting = async (c, e) => {
        ...
        await axios.post(process.env.REACT_APP_BASE_URL +`/admin/quiz/editQuiz/` + sifKviz,
            {
                name,
                image: imag,
                description,
    ...
    return (
        <div>
            ...
            <div className='form'>
                ...
                >
                    <br />
                    <h2 style={{ position: 'relative' }}> Uredi kviz</h2>
                    ...
                    <form style={{
                        ...
                        <Stack direction='row' justifyContent='space-between' alignItems='center' spacing='1rem' >
                            ...
                            <input onChange={handleName} className="input"
                                type="text" name='name' id="name" value={name} placeholder="Zastave Europe" style={{ height: '30px', width: '200px' }} />
                        </Stack>
                        <br />
                        <Stack direction='row' justifyContent='space-between' alignItems='center' spacing='5rem' >
                            ...
                            <textarea onChange={handleDescription} className="input"
                                type="textarea" name='description' id="description" value={description} placeholder="Vrlo poučan i edukativan kviz za sve uzraste" style={{ height: '50px', width: '200px' }} />
                        </Stack>
                        <br />
                        ...
                        <Stack direction='row' justifyContent='space-between' alignItems='center' spacing='3rem'>
                            ...
                            <input onChange={handleDuration} className="input"
                                value={durationSec} type="text" name='durationSec' id="durationSec" placeholder='60' style={{ height: '30px', width: '200px' }} />
                        </Stack>
                        ...
                        <Stack direction='row' justifyContent='space-between' alignItems='center' spacing='4.2rem' >
                            ...
                            <input onChange={handleReviewDuration} className="input"
                                value={reviewDurationSec} type="text" name='reviewDurationSec' id="reviewDurationSec" placeholder='100' style={{ height: '30px', width: '200px' }} />
...
export default ManageSpecificQuiz;